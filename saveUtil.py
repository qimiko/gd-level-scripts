import base64, zlib, plistlib, gzip
...
def decodeLevel(string: str) -> LevelString:
		...
		return LevelString(decompressed) # i also just remembered i have no idea how i found out about this lol
...
if __name__ == "__main__":
	...
	with open('CCLocalLevels.dat', 'r') as localLevels:
		...
		actions = prompt([
						{'type': 'list', 'name': 'action', 'message': 'what do?', 'choices': ['Import', 'Export']},
						{'type': 'list', 'name': 'level', 'message': 'what level?', 'choices': levelNames},
						{'type': 'input', 'name': 'filename', 'message': 'what file?'}])
		selectedLevel = filter(lambda x: x["name"] == actions["level"], gdLevels)
		selectedLevel = list(selectedLevel)[0] # guess who's not subscriptable now!

		# will probably split these into functions at some point but at the same time i don't think that's needed
		if actions["action"] == "Export":
			...
			with open(actions["filename"], 'r') as levelString:
				with open("CCLocalLevels.dat.new", 'w') as newLocalLevels:
					newCC = injectLevel(decoded, levelString.read(), selectedLevel['id'])
